...
export class LiteLlm extends BaseLlm {
  ...
  async *generateContentAsync(llmRequest: LlmRequest, stream?: boolean): AsyncGenerator<LlmResponse, void> {
      ...
      for (const part of content.parts || []) {
        if ('text' in part && part.text) {
          ...
          if (response && 'status' in response) {
            ...
            } else if (response.status === 'error' && 'error_message' in response) {
    ...
    const tools = Object.values(llmRequest.toolsDict).map(tool => {
      ...
      for (const [key, param] of Object.entries(declaration.parameters.properties || {})) {
        properties[key] = {
    ...
    try {
      ...
      try {
        ...
        console.error('Failed to parse Ollama API response as JSON:', jsonError);
      ...
      if (data.message && data.message.tool_calls && data.message.tool_calls.length > 0) {
        ...
        result = {
          content: {
            role: 'assistant',
            parts: [{ text: 'Sorry, I encountered an error processing your request.' }]
          },
          turnComplete: true,
          partial: false
        };
